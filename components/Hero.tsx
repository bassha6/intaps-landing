...
const Card = ({ icon: Icon, title, content, color }) => (
  <div
    className={`space-y-2 ${color} cursor-pointer hover:scale-105 transition-all w-52 mx-auto text-white p-4 rounded-xl`}
  >
    ...
    <div className="p-8 bg-[#0E3E41] relative min-h-screen">
      <div className="flex flex-wrap gap-2 mx-auto items-center justify-center">
        {cards.map((card, index) => (
          <div key={index} className={`md:absolute  ${getPosition(index)}`}>
            <Card {...card} />
          </div>
        ))}
      </div>
...
const getPosition = (index) => {
  const positions = [
    "top-[10rem] left-[10rem] md:-rotate-6",
    "bottom-[10rem] left-[10rem] md:rotate-6",
    "top-[10rem] right-[10rem] md:rotate-6",
    "bottom-[10rem] right-[10rem] md:-rotate-6",
  ];
